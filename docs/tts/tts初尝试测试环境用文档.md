...
|------|------|------|--------|------|
| `text` | string | ✅ | — | 要合成的文字 |
| `text_lang` | string | ✅ | — | 文字语言：`zh` / en / `ja` / `auto` |
| ref_audio_path | string | ✅ | — | 参考音频**绝对路径**（用于声音克隆） |
| `prompt_lang` | string | ✅ | — | 参考音频语言：`zh` / en / `ja` |
| `prompt_text` | string | ❌ | `""` | 参考音频对应的文字（填了效果更好） |
| `media_type` | string | ❌ | wav | 返回格式：wav / ogg / aac / raw |
...
resp = requests.post("http://127.0.0.1:9880/tts", json={
    ...
    "text_lang": "zh",
    "ref_audio_path": "C:/YourPath/tts/v4/八重樱/reference_audios/中文/emotions/【默认】这个身体似乎不会老去，但我想见的人，却都离去了。.wav",
...
设置后，后续 `/tts` 调用可以不再传 ref_audio_path。
...
requests.get("http://127.0.0.1:9880/set_refer_audio", params={
    "refer_audio_path": r"C:/YourPath/tts/v4/八重樱/reference_audios/中文/emotions/【默认】这个身体似乎不会老去，但我想见的人，却都离去了。.wav"
})
```
...
REF_AUDIO = r"C:/YourPath/tts/v4/八重樱/reference_audios/中文/emotions/【默认】这个身体似乎不会老去，但我想见的人，却都离去了。.wav"
...
| 事项 | 说明 |
|------|------|
| ref_audio_path 必须是绝对路径 | 相对路径会报 400 错误 |
